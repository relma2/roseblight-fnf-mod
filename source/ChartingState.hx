...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var stepperPenalty:FlxUINumericStepper;
	var check_isWarning:FlxUICheckBox;
	...
	function addNoteUI():Void
	{
		...
		check_isWarning = new FlxUICheckBox(74, 40, null, null, "isWarnNote", 100);
		check_isWarning.name = 'check_isWarning';

		stepperPenalty = new FlxUINumericStepper(10, 40, 1, 4, 0, 30);
		stepperPenalty.name = 'note_penalty';
		var stepperPenaltyLabel = new FlxText(10, 50, "penalty");

		tab_group_note.add(stepperSusLength);
		tab_group_note.add(stepperSusLengthLabel);
		tab_group_note.add(applyLength);
		tab_group_note.add(check_isWarning);
		tab_group_note.add(stepperPenalty);
		tab_group_note.add(stepperPenaltyLabel);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'isWarnNote':
					{
						trace("warning checked -- " + check.checked);
						if (curSelectedNote == null)
							return;

						if (!check.checked)
						{
							curSelectedNote = [curSelectedNote[0], curSelectedNote[1], curSelectedNote[2]];
							updateNotePenaltyInSection(curSelectedNote);
						}
						else if (check.checked && stepperPenalty.value > 0)
						{
							curSelectedNote = [
								curSelectedNote[0],
								curSelectedNote[1],
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'note_penalty')
			{
				trace("penalty amending -- " + nums.value);
				if (curSelectedNote == null || !check_isWarning.checked)
					return;
				curSelectedNote = [
					curSelectedNote[0],
					curSelectedNote[1],
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null)
		{
			...
			check_isWarning.checked = (curSelectedNote.length >= 4) ? curSelectedNote[3] : false;
			stepperPenalty.value = check_isWarning.checked ? curSelectedNote[4] : 0;
		}
	}
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var daWarning:Bool = (i.length >= 4) ? i[3] : false;
			var daPenalty:Null<Int> = daWarning ? Std.int(Math.max(i[4], 0)) : null;

			var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, false, true, daWarning);
			note.sustainLength = daSus;
			note.penalty = daPenalty;
	...
	function updateNotePenaltyInSection(note:Array<Dynamic>):Void
	{
		var swagNum:Int = 0;
		// floating points gonna floating point
		var EPSILON:Float = 0.000001;
		trace("passed in note  " + note);

		for (i in _song.notes[curSection].sectionNotes)
		{
			trace("relma2 -- section note is " + i);
			var strum:Float = i[0];
			if (strum <= note[0] + EPSILON && strum >= note[0] - EPSILON && i[1] % 4 == note[1])
			{
				_song.notes[curSection].sectionNotes[swagNum] = note;
				trace("found a note to replace");
				break;
			}
